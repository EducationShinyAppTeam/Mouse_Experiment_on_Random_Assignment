    ...
    lapply(names(input), function(x) {
      observeEvent(input[[x]], {
        if(grepl('^btn', x)) {
          ...
          index <- as.integer(gsub("^btn", "", x))
          localData[index, "handPicked"] <<- "Treatment"
  ...
  observeEvent(input$pages, {
    if(input$pages == "summary") {
      ...
      stmt <- boastUtils::generateStatement(
        ...
        verb = "experienced",
        object = "handSummary",
        description = "Summary of Hand-Selected Mice for Comparison",
        response = jsonlite::toJSON(handSummary)
      )
  ...
  observeEvent(input$pages, {
    if(input$pages == "computer") {
      ...
      stmt <- boastUtils::generateStatement(
        ...
        verb = "experienced",
        object = "compSummary",
        description = "Summary of Computer-Selected Mice for Comparison",
        response = jsonlite::toJSON(compSummary)
      )
...
# Call the app
boastUtils::boastApp(server = server, ui = ui, config = list(log = TRUE))